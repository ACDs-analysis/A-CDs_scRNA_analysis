...
# ================== 2) Select key TF genes & preprocessing ==================
...
if (length(missing) > 0) {
  message("The following TF genes were not found in the matrix (please check aliases/species symbols):\n",
...
p_ecm10 <- ggplot(plot_df, aes(x = condition, y = log1pCPM, fill = condition)) +
  ...
  labs(x = NULL, y = "Expression (log1p CPM)",
       title = "Transcription factors in macrophages") +
