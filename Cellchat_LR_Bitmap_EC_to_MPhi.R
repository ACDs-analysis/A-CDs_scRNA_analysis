...
# ========= Consistent with information flow: extract significant Endo → Mac LR pairs =========
SRC <- "Endothelial"     # ← direction changed: endothelial as source
TGT <- "Macrophage"      # ← macrophage as target
PVAL_THRESH <- 0.05
...
# Extract LR pairs from cellchat_results for three groups (Endo → Mac)
...
left_tbl  <- wide2 %>% filter(delta_ACDvsLPS <  0) %>% arrange(desc(delta_abs))  # ACD < LPS
right_tbl <- wide2 %>% filter(delta_ACDvsLPS >= 0) %>% arrange(desc(delta_abs))  # ACD ≥ LPS
...
# Unified color scale / point size
...
p_left  <- mk_panel(df_left,  sprintf("Endothelial → Macrophage — ACD < LPS  [n=%d]", nlevels(df_left$lr_pair)))
p_right <- mk_panel(df_right, sprintf("Endothelial → Macrophage — ACD ≥ LPS [n=%d]", nlevels(df_right$lr_pair)))

H_in <- 0.16 * max(nlevels(df_left$lr_pair), nlevels(df_right$lr_pair)) + 3.0
outdir <- file.path(getwd(), "export_LRdot_Endo_to_Mac_thresh005")
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)
pdf_file <- file.path(outdir, "Endo_to_Mac_LRdot_ACD_vs_LPS_thresh005.pdf")
svg_file <- file.path(outdir, "Endo_to_Mac_LRdot_ACD_vs_LPS_thresh005.svg")

while (dev.cur() > 1) try(dev.off(), silent = TRUE)
grDevices::pdf(pdf_file, width = 9.0, height = H_in, onefile = FALSE, family = "sans")
print(p_left | p_right); dev.off()
svglite::svglite(svg_file, width = 9.0, height = H_in)
print(p_left | p_right); dev.off()
message("Exported:\n", pdf_file, "\n", svg_file)

# ================== Endothelial → Macrophage: LR dotplot (faceted by 6 modules, left/right panels unchanged) ==================
# Assumes you have already obtained df_left / df_right (the long tables generated by prep_plot_df(left_ids/right_ids))
# df_left/df_right must contain: group, lr_pair, ligand, receptor, pathway_name, prob, pval, prob_abs, prob_log10
# This section only: add module → order within module → plot → export

suppressPackageStartupMessages({
  library(dplyr); library(stringr); library(ggplot2); library(patchwork); library(svglite); library(grid)
})

# ---------- 1) 6-module labeling (adapted for Endo→Mac: added MHC/Complement/Notch/PDGF/EGF/APP etc.) ----------
assign_module6_endo2mac <- function(lr_pair, pathway_name, ligand, receptor){
  x <- paste(lr_pair, pathway_name, ligand, receptor, sep=" | ") %>% tolower()
  
  # M1: Inflammation / Chemokine / Immune signaling (wider: MHC/Complement/CSF etc.)
  if(grepl("\\bccl\\d|\\bcxcl\\d|\\bccr\\d|\\bcxcr\\d|\\backr\\d|\\bil\\d|\\btnf\\b|tnfr|il6r|il6st|twea?k|tnfsf|\\bcsf\\d|\\bcsf1\\b|\\bcsf1r\\b|\\bm h c|mhc|h2-|\\bcd1\\b|\\bcd200\\b|complement|\\bc3\\b|\\bc3ar\\b|\\bc5\\b|\\bc5ar\\b|bst2", x)){
    return("M1 Inflammation/Chemokine")
  }
  
  # M2: Adhesion / Contact (ICAM/VCAM/PECAM1/CD34/SELP/SELL/CDH5/CD47 etc.)
  if(grepl("\\bicam\\d|\\bvcam\\d|\\bsele\\b|\\bsell\\b|\\bselp\\b|\\bpecam1\\b|\\bcd34\\b|\\bpodxl\\b|\\bcd47\\b|\\bcd80\\b|\\bcd274\\b|\\bcdh5\\b|\\bsdc\\d|\\bitgal\\b|\\bitgb2\\b", x)){
    return("M2 Adhesion/Contact")
  }
  
  # M5: Guidance / SEMA–NRP (you have many Sema3/4 + Nrp/Plxn on the right)
  if(grepl("\\bsema\\d|\\bnrp\\d|\\bplxn", x)){
    return("M5 Guidance/SEMA–NRP")
  }
  
  # M4: Angio / Growth factor (PDGF/EGF/EDN/NOTCH/BMP/ANGPT/VEGF/IGF etc. also assigned to growth/repair)
  if(grepl("\\bvegfa\\b|\\bflt\\d|\\bkdr\\b|\\bangpt\\d|\\bangptl\\d|\\bigf\\d|\\bkitl\\b|\\bkit\\b|\\bnampt\\b|\\binsr\\b|\\bosm\\b|\\bosmr\\b|\\bpdgf\\b|\\begf\\b|\\begfr\\b|\\bedn\\d|\\bednra\\b|\\bnotch\\d|\\bdll\\d|\\bbmp\\d|\\bavcrl1\\b|\\bhbegf\\b|\\bapp\\b", x)){
    return("M4 Angio/Growth")
  }
  
  # M3: ECM–Integrin structural (COL/LAM/FN1/THBS/SPP1 + ITGA/ITGB; and CD44–ECM axis)
  if(grepl("\\bcol\\d|\\blam[a-z0-9]*\\b|\\bfn1\\b|\\bspp1\\b|\\bthbs\\d|itga\\d|itgb\\d|itgav|itgal|itgb2|\\bcd44\\b|\\bdag1\\b", x)){
    return("M3 ECM–Integrin")
  }
  
  # M6: Homeostasis / Other (fallback)
...
add_module6 <- function(df){
  df %>%
    mutate(
      module6 = mapply(assign_module6_endo2mac, lr_pair, pathway_name, ligand, receptor),
...
# ---------- 2) Order within module: preserve your original ordering of lr_pair (already sorted by delta_abs) ----------
reorder_within_module <- function(df){
  ...
  df %>%
    ...
    group_by(module6) %>%
    mutate(lr_pair2 = factor(as.character(lr_pair), levels = unique(as.character(lr_pair)))) %>%
    ungroup()
}
df_left  <- reorder_within_module(df_left)
df_right <- reorder_within_module(df_right)

# ---------- 3) Unified color scale / point size ----------
cmin <- quantile(c(df_left$prob_log10, df_right$prob_log10), 0.05, na.rm=TRUE)
cmax <- quantile(c(df_left$prob_log10, df_right$prob_log10), 0.95, na.rm=TRUE)
cmid <- median(c(df_left$prob_log10, df_right$prob_log10), na.rm=TRUE)
cap99 <- quantile(c(df_left$prob_abs, df_right$prob_abs), 0.99, na.rm=TRUE)
...
# ---------- 4) Faceted plotting (module facets) ----------
...
p_left  <- mk_panel_module6(df_left,  sprintf("Endothelial → Macrophage — ACD < LPS  [n=%d]",  nlevels(df_left$lr_pair2)))
p_right <- mk_panel_module6(df_right, sprintf("Endothelial → Macrophage — ACD ≥ LPS [n=%d]", nlevels(df_right$lr_pair2)))

# ---------- 5) Export (wide figure) ----------
H_in <- 0.16 * max(nlevels(df_left$lr_pair2), nlevels(df_right$lr_pair2)) + 3.0

outdir <- file.path(getwd(), "export_LRdot_Endo_to_Mac_thresh005")
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

pdf_file <- file.path(outdir, "Endo_to_Mac_LRdot_ACD_vs_LPS_module6.pdf")
svg_file <- file.path(outdir, "Endo_to_Mac_LRdot_ACD_vs_LPS_module6.svg")
...
message("Exported (Endo→Mac 6-module faceted):\n", pdf_file, "\n", svg_file)