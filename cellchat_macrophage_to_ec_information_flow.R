...
# ================== Mac→Endo information flow: sorted by LPS proportion from low to high ==================
suppressPackageStartupMessages({
  library(dplyr); library(tidyr); library(ggplot2); library(patchwork)
})

compare_groups <- c("ACD", "LPS")
source_cell    <- "Macrophage"
target_cell    <- "Endothelial"
min_flow_threshold <- 0
top_n <- NA
outfile_pdf <- sprintf("MacEndo_Flow_%s_vs_%s_sorted_by_LPS.pdf",
                       compare_groups[1], compare_groups[2])

# ---- Helper functions ----
.compute_pathway_flow <- function(cellchat_obj, source="Macrophage", target="Endothelial") {
  if (is.null(cellchat_obj)) return(tibble(pathway=character(), flow=numeric()))
  df <- tryCatch(subsetCommunication(cellchat_obj, sources=source, targets=target),
  ...
  df %>%
    mutate(pathway = .data[[path_col]]) %>%
    group_by(pathway) %>%
    summarise(flow = sum(prob, na.rm = TRUE), .groups = "drop")
}

# ---- Summarize data for both groups ----
avail_groups <- intersect(compare_groups, names(cellchat_results))
stopifnot("Insufficient groups for comparison" = length(avail_groups) >= 2)

flow_df <- bind_rows(lapply(avail_groups, function(g){
  tibble(group=g) %>%
    bind_cols(.compute_pathway_flow(cellchat_results[[g]], source_cell, target_cell))
}))

flow_df <- flow_df %>%
  group_by(pathway) %>%
  filter(max(flow, na.rm = TRUE) > min_flow_threshold) %>%
  ungroup()

# ---- Relative proportions (ACD + LPS = 1) ----
wide <- flow_df %>%
  pivot_wider(names_from = group, values_from = flow, values_fill = 0) %>%
  select(pathway, all_of(compare_groups))
wide$.sum <- rowSums(as.matrix(wide[, compare_groups, drop=FALSE]), na.rm = TRUE)

rel_long <- wide %>%
  pivot_longer(cols = all_of(compare_groups), names_to="group", values_to="flow") %>%
  mutate(rel = ifelse(.sum > 0, flow/.sum, 0)) %>%
  select(pathway, group, rel)

# ---- Sort: by LPS relative information flow from low to high ----
order_tbl <- rel_long %>%
  filter(group == "LPS") %>%
  arrange(rel) %>%
  select(pathway)
...
if (!is.na(top_n) && top_n > 0 && nrow(order_tbl) > top_n) {
  keep_paths <- order_tbl$pathway[1:top_n]
  order_tbl <- order_tbl %>% slice_head(n = top_n)
  flow_df  <- flow_df  %>% filter(pathway %in% keep_paths)
  rel_long <- rel_long %>% filter(pathway %in% keep_paths)
}
...
# Left plot: relative information flow (stacked, ACD+LPS=1)
p_rel <- ggplot(rel_long, aes(x = rel, y = pathway, fill = group)) +
  ...
  scale_x_continuous(limits = c(0,1), expand = expansion(mult = c(0.00,0.02))) +
  scale_fill_manual(values = comp_cols) +
  labs(x = "Relative information flow (ACD + LPS = 1, sorted by LPS ↑)", y = NULL) +
  theme_minimal(base_size = 10) +
  theme(legend.position = "top",
        axis.text.y = element_text(color="#e67e22"),
        panel.grid.major.y = element_blank())

# Right plot: absolute information flow
